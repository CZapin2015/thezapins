  ...
  map.on('load', () => {
    ...
    map.getStyle().layers.forEach(layer => {
      if (layer.type === 'symbol' && (layer.id.includes('label') || layer.id.includes('place'))) {
        try { map.setPaintProperty(layer.id, 'text-color', 'hsl(38, 35%, 62%)'); } catch(e) {}
      }
    });
...
function updateCountdown() {
  ...
  if (diff <= 0) return;
  // Use start of today for days so the count matches calendar days remaining
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const days = Math.floor((wedding - todayStart) / 86400000);
